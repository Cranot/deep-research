...
BEDROCK_PROMPT = """Have we reached bedrock? Be CONSERVATIVE - only say YES if truly atomic.

CURRENT QUESTION: {question}
DEPTH SO FAR: {depth} levels

Bedrock means the question is ATOMIC - it cannot be meaningfully decomposed further.

Check ALL of these:
1. Is this question SIMPLE enough to answer in one paragraph?
2. Does it have NO hidden sub-questions worth exploring?
3. Is it purely ONE of: DEFINITIONAL (meaning), EMPIRICAL (data), VALUES (preferences), CONSTRAINTS (limits)?
4. Are we CIRCLING back to earlier questions?

Say YES ONLY if the question is truly atomic AND simple.
Complex questions about productivity, success, learning, etc. are NOT bedrock.

On a line starting with "BEDROCK:", write YES or NO.
If YES, on a line starting with "TYPE:", write: DEFINITIONAL, EMPIRICAL, VALUES, or CONSTRAINTS."""
...
class SocraticEngine(Recipe):
    ...
    async def run(
        ...
        for depth in range(self.MAX_DESCENT_DEPTH):
            ...
            console.print(
                f"[dim]  Found {len(all_assumptions)} assumptions, using top {len(assumptions)}[/dim]"
            )
            ...
            async def challenge_one(assumption: Assumption) -> None:
                ...
                completed += 1
                console.print(
                    f"[dim]  Challenged {completed}/{total}: {assumption.text[:40]}...[/dim]"
                )
            ...
            # Phase 3: Check for bedrock (skip check on first level for complex questions)
            console.print("[dim]  Checking for bedrock...[/dim]")
            bedrock_result = await provider.generate(
                BEDROCK_PROMPT.format(question=current_question, depth=depth + 1),
                model.model,
            )
            ...
            assumptions_text = "\n".join(f"- {a.text} ({a.status.value})" for a in assumptions)
    ...
    def _parse_bedrock(self, output: str) -> tuple[bool, BedrockType]:
        ...
        for line in output.split("\n"):
            ...
            if line_lower.startswith("bedrock:"):
                value = line_lower.split(":", 1)[1].strip()
                is_bedrock = "yes" in value and "no" not in value
            elif line_lower.startswith("type:"):
                value = line_lower.split(":", 1)[1].strip()
                # Match type keywords anywhere in the value
                if "definitional" in value or "definition" in value or "meaning" in value:
                    bedrock_type = BedrockType.DEFINITIONAL
                elif "empirical" in value or "data" in value or "measurable" in value:
                    bedrock_type = BedrockType.EMPIRICAL
                elif "values" in value or "preference" in value or "subjective" in value:
                    bedrock_type = BedrockType.VALUES
                elif "constraint" in value or "limit" in value or "fixed" in value:
                    bedrock_type = BedrockType.CONSTRAINTS
...
"""
        ...
        if all_assumptions:
            for a in all_assumptions:
                status_icon = {"solid": "✓", "questionable": "?", "flawed": "✗"}.get(
                    a.status.value, "•"
                )
...
### Flawed Assumptions Removed
"""
            if flawed_assumptions:
                for fa in flawed_assumptions:
                    output += f"- ✗ {fa}\n"
            else:
